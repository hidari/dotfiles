...
# handoff-sentinel のコンテキスト推定が advisor 呼び出しで executor 2 回分を合算して誤発火する
...
実測 (transcript の該当エントリ `msg_011CfPcPiNJbb2uhvFkj8LCG`、2026-09-25T05:33:00Z):
...
通知は PostToolUse で鳴るため、advisor を含む応答の直後のツール呼び出しで発火する。既定値 (`DEFAULT_CONTEXT_WINDOW_TOKENS = 1_000_000`、`DEFAULT_CONTEXT_THRESHOLD_PCT = 50`) では、占有量が 25% を超えたあたりから advisor を呼ぶたびに誤発火しうる。
