...
- [ ] statusline-command.sh のアカウント分離
  - [ ] config dir / アカウントタグ / Keychain service 名 / `.claude.json` パスを導出する純粋関数を切り出す
  - [ ] `# ヘルパー関数` `# メイン処理` のマーカーを入れ bats から marker-slice で source できるようにする
  - [ ] `CACHE_FILE` をアカウントタグ別のパスにする
  - [ ] Keychain service 名を導出値にし、item 不在時は他方へフォールバックせず `--%` に落とす
  - [ ] line 1 の先頭にアカウント（メールアドレス）を色付きで表示する
- [ ] `.zshrc` に起動関数を追加
  - [ ] `_claude_task_list_notice` / `claude` / `claude-hamiltonian` を追加
  - [ ] ディレクトリ不在時に非ゼロで終了し `claude` を起動しないこと
  - [ ] `claude` の関数化が再帰しないこと（`command claude` を使う）
  - [ ] 個人アカウント側が `CLAUDE_CONFIG_DIR` を設定しないこと
- [ ] `bootstrap.sh` の `SYMLINK_PAIRS` に hamiltonian 側の 3 行を追加
- [ ] drift 検出テストを追加
  - [ ] `.claude/` を target に持つ pair に、同じ source を持つ `.claude-hamiltonian/` の pair が存在することを検証する
  - [ ] statusline の導出ロジックのテスト（既定 / 指定時の service 名・タグ・`.claude.json` 探索先、2 つの config dir でキャッシュパスが異なること、item 不在時にフォールバックしないこと）
  - [ ] 各テストが変異注入で赤くなることを確認する
- [ ] 検証
  - [ ] `bats scripts/tests/` が緑
  - [ ] `./bootstrap.sh --dry-run` で意図した symlink だけが出る
  - [ ] 両アカウントを実起動して live smoke（shell-out と外部 CLI のオーケストレーションなのでユニットテスト緑では完了としない）
