...
tgrep を環境から撤去したので、報告の工程を実施しないまま閉じる。撤去は、rg で検索の速度が足りるという判断による。
ラッパーと hook も同じ変更で消えたので、上流の修正を ISSUE-102 の運用判断を見直す契機として待つ理由も無くなった。

上流へは報告していない。1件目と2件目の実測は ISSUE-102 の spec に残る。3件目が参照する `home/.claude/references/observation.md` の tgrep 節は撤去と同じ変更で削除したので、削除前の本文はその PR のコミット履歴から引く。
