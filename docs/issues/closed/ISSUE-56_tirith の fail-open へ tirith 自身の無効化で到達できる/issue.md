...
- [x] 包み込み構文の扱いを決める。フック側で前処理はしない。この層の射程を docstring へ
      1 箇所書き、残余は受容する
- [x] 検査が沈黙している状態を検出する層を持つか決める。持つ。設計と実装は ISSUE-59 が担当する
- [x] tirith 自身の無効化に当たるコマンドをフック側の denylist で止めるか決める。作らない
      (下の判断節が理由を持つ)
...
### 切り出したもの: 検査が沈黙している状態を検出する層

ISSUE-59 として起票した。tirith 側は config-guard に置けない (CI に tirith が無く、skip 条件が
検出対象と一致して自己敗北する)。SessionStart での behavioral canary が要り、設計と実装の
範囲が別 Issue の大きさになるので分けた。

**起票時に「ISSUE-55 で apm 側に同じ機構を入れたので形は決まっている」と書いたのは誤り
だった。** PR #183 のレビューで apm 側の機構を測ったところ、配置漏れの検出はフックが
パースできた形にしか到達しない。つまり shim だけが担当する形 (包み込み・変数展開・xargs)
では配置漏れが無音の素通りのまま残る。実測表は ISSUE-55 の本文にある。

形が決まっているどころか、apm 側も同じ層を必要としている。ISSUE-59 は両者を担当する。

### fail-open 通知は暫定である

判定を出さず警告文だけを載せる形にしたが、告げているのはコマンド単位である。「tirith が
入っていない」はセッション単位の事実なので、tirith 不在のマシンでは同じ文が Bash 呼び出しの
たびに積む。恒久策は ISSUE-59 の層で、そちらができたらこの通知は畳む。
