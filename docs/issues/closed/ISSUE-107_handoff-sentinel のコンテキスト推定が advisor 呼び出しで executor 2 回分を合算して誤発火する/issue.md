...
- [x] 上の実測値の形 (3 段で advisor を挟むもの / 1 段のもの / `iterations` 無し) を `scripts/claude-hooks/tests/test_handoff_sentinel.py` に fixture として入れ、推定値を exact で pin する。合算に戻す変異で赤くなることを確かめる
      `message` の2段と空の配列も足した。推定値は、しきい値を100%にして窓を推定値ちょうどにすると鳴り、1つ増やすと鳴らないことで pin した。合算に戻す変異のほか、最初の段から探す、飛ばす型や数える型を減らす、段の値の検証を外す、数えるフィールドを増減するなど16種の変異を1つずつ入れ、すべて狙ったテストが赤くなった
