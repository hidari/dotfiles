...
- F01 は監査の案の「文体にだけ効かせ」を避け、「技術的な正確さと検証の深さは、言語設定の口調に左右されないこと」の1文にして当てた。「効く」は ISSUE-108 が翻訳調の比喩として扱う語で、肯定形と否定形で同じ制約を2回言う形もマージ前ゲートで畳んだ
- F02 は2.1.282のバイナリでも、古い2行が0件、新しい1行が2件だった (対照の `AskUserQuestion` は72件)。記述は「まとまった」という推定を避けて観測した形で書き、この環境の Workflow が settings の `ultracode` の経路で opt-in されていることも足した
- F04 と L6 は採らない (両方据え置き、ユーザー判断)。2.1.282で effort の既定を決める関数を読むと、settings の `ultracode` が true の間は user settings の `effortLevel` を読まず、既定を xhigh に固定する。F04 だけを当てても実効は変わらない。コードを読んだ結論で、live では確かめていない
- F05 は監査のパッチに、コンテキストの通知が出ていることの対照を足して当てた。変異2本 (通知へ推定値を戻す / 合算から1フィールドを落とす) は、それぞれ狙いのテストだけを赤にした
- F07 の理由は推定どおり「素のテキストで読まれる前提」で確定した (ユーザー判断)。10行目も「順序に意味があるときだけ使う」へ言い切った
- F08 は削除で確定した。数字の前後の空白 (新しく書く文面では強調以外で入れない、というユーザーの要望) と、文中の改行の強制は ISSUE-108 へ切り出した
- L1〜L5・L7〜L9 は採った。L4 は多層防御の行と、その子の XSS / SQLi の行、HTTPS の行を削った
- L10 は、監査が挙げた4本に限らず references 全8本で数え直した。delegation.md・git-workflow.md・host-environment.md・premises.md にも同じ再掲があり、git-workflow.md の「push の完了確認」は observation.md の「成否が隠れる」項と同文だったので参照に置き換えた。消えたり書き換えたりした77文は検算スクリプトで規範側の対応する文と突き合わせ、Fable のレビューを1回通して指摘12件のうち10件 (うち2件は一部) を直した。直さなかったのは、規範が無条件なので canonical 側に寄せた checkout の条件、過去形の事例として残した `.cache` の相対パスの実測、ユーザーの判断で詰めている数字の前後の空白の3件。触った references の文中の改行も結合した
- 挙動での確認はしない (ユーザー判断)
- 常時層は26349Bから26016Bへ縮んだ。予算は26849Bから26480Bへ下げ、約500Bの余白を保った

## タスク

- [x] 起票時点の未コミット変更 (tgrep の撤去、CLAUDE.md の整理、Markdown の規範の移設) を先にコミットする
- [x] F01・F03・F09・F10・F11 を `home/.claude/CLAUDE.md` へ反映し、config-guard の scan と pytest の両方を通す
- [x] F02 を `home/.claude/references/delegation.md` へ反映する
- [x] F04 の effort の段を決めて `home/.claude/settings.json` へ反映する (据え置きに決めた)
- [x] F05 を hook とテストへ反映する。ISSUE-67 と同じ関数を触るので、後から入る側も数値を含まないことを pin するテストを残す
- [x] F07 を反映し、F08 は削除か検査での強制かを決めてから反映する
- [x] F12 と F13 を `CLAUDE.md` へ反映する
- [x] L1 から L10 の採否を決め、採るものは同じ PR で直す
- [x] 文面の変更をモデルの挙動で確かめるかを決める。確かめるなら 1 件ずつ、`empirical-prompt-tuning` skill を名指しで使う (確かめないに決めた)
- [x] 反映が済んだら `.cache/prompt-audit/` を消す

## 関連

ISSUE-67: handoff-sentinel のコンテキスト閾値の通知が再武装しない件。F05 が同じ関数 (`_context_notices`) とテストを触る

ISSUE-108: F08 (数字の前後の空白) と文中の改行の強制を切り出した先
