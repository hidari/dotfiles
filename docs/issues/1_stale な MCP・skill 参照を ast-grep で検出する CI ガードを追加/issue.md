...
- [ ] 検出ルールの対象を確定する（最低: (a) legacy un-prefixed `mcp__chrome-devtools__` 形、(b) `home/.claude/skills/*/SKILL.md` の `allowed-tools` にある非実在ツール名 = canonical built-in でも有効な `mcp__plugin_..._...__*` 形でもない bare word）
- [ ] ast-grep（YAML 構造を見るなら）か grep（軽量正規表現で足りるなら）かを選定する。`ast-grep-practice` skill の指針に従う
- [ ] ルールを実装し、既知の good/bad ケースでテストする（exact 検出 + false positive 抑制）
- [ ] `.github/workflows/test.yml` に組み込む（bats / ruff / gitleaks と並ぶ check として）
- [ ] 「実在ツール名」をどう判定するか（ソース・オブ・トゥルース）をルールに与える方法を決める
