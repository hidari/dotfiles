...
常時層から 516B が消え、rules 側は 1,397B になる (設計ドキュメントの項を含めた後の値)。
N 文脈のうち M が `.md` を Read するとき、常時層のコストは全文脈に掛かり
scoped は Read した文脈にだけ掛かるので、得は `516N - 724M` (移設 2 項目時点の rules は 724B)。
M/N が 0.71 を下回れば得になる。

このリポジトリは Issue 駆動でほぼ全文脈が `.md` を読むため単体では損に近いが、
CLAUDE.md はグローバル設定であり、コード中心の他プロジェクトでは得になる。

### MUST GLOBAL を MUST へ戻した

H1-13 は `[MUST GLOBAL]` ラベルを持っていた。「プロジェクトごとの CLAUDE.md より優先される」
という宣言は、両方が同時に見えている状況を前提とする。プロジェクト CLAUDE.md は常時ロード
されるので、それを上書きする側が条件付きロードだと優先順位の宣言そのものが成立しない。

履歴を追うと、導入時 (2026-05-26 `1415d94`) にはラベルが無く、
2026-08-19 の族分類再編 (`13173df`) で付与されていた。3 ヶ月ラベル無しで運用された後の
副作用なので、降格ではなく元の強度へ戻す操作にあたる。

導入コミットの本文は「`ls` の二重管理で必ず drift するため」であり、
実害は drift に限定される。他の MUST GLOBAL 項目 (データ保護・作業の消失・秘密の漏洩) と
並べると 1 段軽い。

**強度ラベルの降格はどの検査も見ない。** CLAUDE.md 自身が
「識別子の照合で情報が落ちていないかは見えるが、射程の広がりや強度ラベルの降格は見えない」
と書いているとおりで、この判断は記録でしか残せない。
...
### references ポインタの射程を絞った

rules の冒頭ポインタは既存 2 枚に倣って
「手当ての詳細と一次実測は `~/.claude/references/canonical.md` が持つ」と書きかけたが、
同時に移した設計ドキュメントの項について canonical.md は何も持たない
(全 references を grep して 0 件を確認した)。

宣言の射程が実態より広くなるので
「ディレクトリツリーと静的検査についての手当ての詳細と一次実測は」へ絞った。
これは 4 点検算の 3 番目 (書いた宣言の範囲が実態より広くないか) が、
移設先の側でも同じ形で発生することを示している。
...
3. **MUST GLOBAL の優先順位宣言。** プロジェクト CLAUDE.md より優先されるという宣言は、
   両方が同時に見えている状況を前提とするので、条件付きロードへ落とすと成立しない
4. **前提と方針。** 開発スタイル・公開範囲の確認・個人情報・CI コスト・作業プロトコル。
...
| 移した規範 | 移設先 | バイト |
|---|---|---|
| H1-13 ディレクトリツリー + H2-01 静的検査 | rules/markdown-practices.md | 516 |
| 設計ドキュメントの継続更新 | rules/markdown-practices.md | 622 |
| H6-07 regex parse | rules/testing-practices.md | 439 |
| カバレッジは分岐を優先 | rules/testing-practices.md | 138 |
| コミット prefix 一覧 + `(wip)` | references/git-workflow.md | 796 |
| references ポインタの定型文重複 6 箇所 | 前文へ 1 回 | 117 |

常時層は 29,182B から 26,554B へ (2,628B 減)。scoped は 4,552B から 6,526B へ。
`ALWAYS_LOADED_BUDGET_BYTES` も 26,554 へ下げた
