...
前節の対照設計に従って測った。subagent を一切起動していないので、このセッションに記録された
`path_glob_match` は全て main loop のものになる。基準線はセッション開始時点で 0 件、
数えるときは `file_path` で絞った。Claude Code は 2.1.238 (`CLAUDE_CODE_EXECPATH` で確認)。
...
`**/tests` や `**/test` のような形が、そのディレクトリ配下のファイルを Read したときに
発火することを意味する。glob の corpus 問題で「広い 1〜2 個へ寄せる」を採るときの材料になる。
...
`home/.claude/` 配下へ書き込むと skills の再スキャンが走り、`home/.claude/skills/`
(apm の deploy 先) が `home/` にスコープされた skill として登録される。16 個の description が
その場でコンテキストへ入る。常時ロード層の予算とは別勘定だが、`home/` を触るセッションでは
毎回払っている可能性がある。
...
- [x] `paths` 付き rules の発火を、subagent を起動しない単一文脈で測り直す
      (発火するのは Read ツールのみ。Bash の読み書き・Write・Edit・stat・拒否されたコマンド・
      出力にだけ名前が現れる場合はいずれも発火させない。照合対象は Read の `file_path` 引数で、
      実在しないパスでも発火する。結果は「発火の条件を単一文脈で測り直した」節)
...
- [ ] `home/.claude/skills/` が `home/` スコープの skill として登録される件のコストを測る
      (apm の deploy 先が directory-scoped skill として二重に列挙される。16 個の description を
      毎回払っている可能性がある。`instruction_budget` は常時ロード層しか見ないので数字に出ない)
