...
- [ ] 予算が詰まったときの移設を skill へ切り出す
      (検出は config-guard の内訳で足りるが、どこを何へ移すかの判断と手順は毎回やり直している。
      判定基準「トリガが失敗側に無い規範はスコープできない」と references / rules の使い分け、
      移設後の検証手順をまとめる。置き場所が dotfiles か agentic-coding-tools かは着手時に決める)
