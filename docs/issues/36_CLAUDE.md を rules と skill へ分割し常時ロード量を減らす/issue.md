...
`paths` 付き rules には穴がある。compact から次に一致ファイルへ触るまでの間、その規範は不在になる。
穴の広さはどの経路が発火させるかで決まる。実測は「発火経路を実測した」節。
...
current だったため。穴が開くのはこの経路、つまり対象ファイルを読まずに書く場面に限られる。

そこで効く規範を scoped rules へ置くと沈黙する。新規ファイルの生成規約と
テストを書き始めるときの規範がこれに当たる。
