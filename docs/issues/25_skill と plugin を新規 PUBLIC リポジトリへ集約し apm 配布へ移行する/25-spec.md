...
この節と下の「既定の設定ディレクトリ側の実測」節は実環境での観測で、上の前置きが宣言する
隔離環境の保証は及ばない。この節が観測したのは追加の設定ディレクトリ側のセッション 1 本で、
既定の設定ディレクトリ側は構成が違うため下の節へ分けた。

決め手になったのは skill 起動時に表示される base directory である。これは Claude Code が
どのパスから読んだかを自ら申告する値で、読み込み経路そのものを指す。バンドル入口 3 件と
...
元から張られない。不在が情報を持つのは cache だけである。ただし cache の不在を供給元の判別に
使うには前提が 1 つ要る。下の「既定の設定ディレクトリ側の実測」節で指摘する。
...
挙動は変わらない見込みだった。下の節で実際に確かめた。

### 既定の設定ディレクトリ側の実測

既定の設定ディレクトリのセッションで `dev-workflow` の component (`issue-scoped-artifacts`) を
引き、base directory が apm の配置先を指すことを確認した。上の節が残していた未観測の構成は
これで解消した。

判別の根拠は cache の不在ではなく registry の不在に置いた。既定側の `known_marketplaces.json`
は 5 件すべてが github / git source で directory source が 0 件、`installed_plugins.json` は
17 キー 19 エントリすべてが同じ 5 marketplace 由来で、移行した 3 パッケージのエントリが 0 件
である。

この 0 件には対照を 2 本並べる。対照 1 は同じ検索を実在する名前へ当てると非空になること。
対照 2 は同じ parse を追加の設定ディレクトリ側へ当てると directory source の marketplace が
1 件、3 パッケージのエントリが 3 件見つかることである。後者は検出そのものが働いていることを
示すので、既定側の 0 件は「そもそも見ていない」ではない。

上の節は「不在が情報を持つのは cache だけである」と書いているが、その対照はここで再利用しな
かった。上の節は `plugins/marketplaces/<名前>` について「directory source では元から作られない」
と source 種別の非対称を既に認めている。同じ非対称が cache に無いとは限らない。実際
installLocation は、外部 marketplace が `plugins/marketplaces/<名前>` を指すのに対し private
marketplace ではその外を指す。cache の不在を根拠にするには「directory source も同じように
cache へ実体化する」という前提が要り、この前提は確かめていない。上の節の結論を否定するもので
はない。registry の不在はこの前提を必要としない。

両方の設定ディレクトリの `skills` は symlink で、実体はどちらもリポジトリ配下の
`home/.claude/skills` を指す (`bootstrap.sh` の `APM_SYMLINK_PAIRS` が 2 本とも宣言しており、
実測でも同一 inode を指す)。base directory が申告するのは読み込みに使ったパスであって物理
ツリーではないので、2 つの観測は同じ実ファイルを読んでいる。違うのは周辺構成で、追加側は
registry のエントリを保持していたため registry による消去法が使えなかったのに対し、既定側は
apm 経路しか残らない。その構成でも解決したことが新しい事実である。

同じ実ファイルを読む以上、上の節が行った本文の突き合わせは既定側へそのまま転移し、再実施は
要らない。一方で同一なのは `skills` の配下だけで、plugin 由来の skill が置かれる `plugins` は
symlink されておらず、両設定ディレクトリで内容が異なる (marketplace の集合が違い、共有する
plugin でも pin された版が違う)。本節の結論は `skills` の配下にのみ及ぶ。
...
| 既定の設定ディレクトリ側が未観測 (解決済み) | 供給切り替えを観測したのは追加の設定ディレクトリ側で、既定側は symlink と cache に加えて registry のエントリも落としており、この構成はどのセッションでも観測していなかった | 既定の設定ディレクトリのセッションで確認した。base directory が apm の配置先を指し、対照 2 本で判別力も確かめてある。詳細は「既定の設定ディレクトリ側の実測」節 |
