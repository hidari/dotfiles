...
- [ ] CLAUDE.md (global + project) の全ルールを「機械検出可能 / 手順判断」で分類する
- [ ] 機械検出可能なものを ast-grep ルールへ移す候補としてリスト化する (rule-tests のペアも設計)
- [ ] 各ルールを rules/ へ追加し rule-tests/ に valid / invalid を書く
- [ ] 移したルールを CLAUDE.md から削除する
- [ ] ast-grep test と scan が緑であることを確認する
