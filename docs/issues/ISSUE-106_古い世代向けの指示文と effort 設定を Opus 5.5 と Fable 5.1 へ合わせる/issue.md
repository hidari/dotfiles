...
- 起票時点 (2026-09-24) の作業ツリーには、このブランチの未コミット変更が残っていた (tgrep の撤去、`home/.claude/CLAUDE.md` の整理、Markdown の規範の rules への移設)。下の F06・F07・F08 はその変更が作った行を対象にするので、先にそちらをコミットしてから着手する
- その未コミット変更は `scripts/config-guard/src/config_guard/instruction_budget.py` の `BUDGET_RAISES` から 2026-09-13 のエントリ (tgrep の規範のための引き上げ) を消しているが、`ALWAYS_LOADED_BUDGET_BYTES` は 27753 のまま残している。コミットするときに config-guard の pytest で予算の検査が通るかを確かめる
...
- memory の `feedback-prefer-tgrep-over-grep.md` は tgrep を既定とする前提のまま残っている。tgrep の撤去をコミットするときに一緒に直す (CLAUDE.md の完了前 2)
...
- F06 (71 行目)「検索は Grep ツールや素の `grep` ではなく `rg (ripgrep)` を既定にすること。」。メインセッションのツール一覧に Grep は無い。逆に `feature-dev:code-reviewer` のように Grep はあって Bash が無い subagent では、唯一の検索手段を禁じることになる。この repo はマージ前にその code-reviewer を必須にしており、常時ロードの層は subagent にも載る。「Bash で検索するときは素の `grep` ではなく `rg (ripgrep)` を既定にすること」へ置き換える
...
- F07 (7 から 9 行目) の強調・区切り線・絵文字・テーブルの禁止。理由の無い禁止の形で、装飾が多かった世代向けの書き方。この rule は `*.md` を Read した時点でロードされるので、ファイルの規約のつもりが会話の応答の書式まで縛る。ファイルに限った理由付きの肯定形として「Markdown ファイルは、レンダリングされない素のテキストで読まれる前提で書く。強調（`**これ**`）・区切り線（`---`）・過剰な絵文字・テーブルは素のテキストでは読みづらいので使わず、構造は見出しとリストで示す」と「README.md などブラウザ等でプレビューされる見込みがあるファイルに限り、テーブル記法を使ってよい」へ置き換える。理由は 9 行目の条件から推定したもので、違っていたら差し替える
- F08 (14 行目)「強調したい場合を除いて、数字の前後に空白を入れないこと」。指示ファイル群は少なくとも 95 行でこれを破っている (行単位で数えたので実際の箇所数はもっと多い。`home/.claude/CLAUDE.md` の見出し 3 本を含む)。例は規則より強く効くので、この行はモデルへの合図にならない。同じファイルの 22 行目は、静的に検査できる規則を linter へ回すよう求めている。削除するか、採るならファイル群を一括で整形して pre-commit の検査で強制するかを決める
...
- [ ] L1 から L6 の採否を決め、採るものは同じ PR で直す
- [ ] 文面の変更をモデルの挙動で確かめるかを決める。確かめるなら 1 件ずつ、`empirical-prompt-tuning` skill を名指しで使う
- [ ] 反映が済んだら `.cache/prompt-audit/` を消す

## 関連

ISSUE-67: handoff-sentinel のコンテキスト閾値の通知が再武装しない件。F05 が同じ関数 (`_context_notices`) とテストを触る。閾値そのものの前提の見直しは、本 Issue からあちらへ判断材料として追記した。変わったのは判断材料の内容で、あちらの着手順序は変えない
