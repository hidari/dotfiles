...
- `UNMANAGED_HOME_FILES`: `home/` 配下だが意図的に symlink しないファイルの allowlist。各行にコメントで理由を書き自己文書化する。canonical な定義はこの bats 配列であり、本 spec の列挙は設計時点のスナップショットである。
- reverse テスト本体: `uncovered_symlink_targets | sort` と `UNMANAGED_HOME_FILES` を sort したものの完全一致を要求する。不一致時は差分を `diff` で出して FAIL する。
...
- 変異注入で pin の有効性を確認する（CLAUDE.md MUST）。同一ファイルに未コミット編集があるため復元は `git checkout` ではなくバックアップ復元か変異箇所のみ戻す。
  ...
  - (c) `UNMANAGED_HOME_FILES` から 1 件削除 → 集合不一致で FAIL する。
