...
- `git_source`: settings.json の内容を git（staged を優先し HEAD にフォールバック）から読む。working tree は決して読まない。
...
settings.json は skip-worktree のため working tree = live superset である。検査器が working file を読むと個人トグルや `/Users` パスを誤検出する。これを防ぐため `git_source` は次の順で git から内容を取得し、working file は決して読まない。

1. staged（`git show :home/.claude/settings.json`）があればそれを使う。pre-commit では cacheinfo dance で差し替えた committed blob がここに入る。
2. staged が無ければ HEAD（`git show HEAD:home/.claude/settings.json`）を使う。CI のチェックアウトでは working == HEAD == committed なので正しい。
...
- git_source が staged を優先し、working file を読まないこと（staged/HEAD/working で内容が異なる一時 git リポジトリで検証）。
