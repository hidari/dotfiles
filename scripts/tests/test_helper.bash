...
run_in_dir() {
    local dir="$1"
    shift
    # 実在するディレクトリでなければ弾く。bash では cd "" が rc 0 を返してカレントに留まるので、
    # 呼び出し側が変数を渡し忘れるとテストがリポジトリのチェックアウト上で走る。期待値を持つ
    # テストは値が外れて赤くなるが、2 つの実装を突き合わせるだけの対照は両方が同じ場所を見て
    # 一致するため緑になる。作っていない入力について緑を返す形なので、cd の rc では捕まらない。
    #
    # 空文字を別に検査しない。-d は空文字にも偽を返すので同じ枝で捕まる (両方を並べた形は
    # 変異で確かめたところ、-z を外しても全件緑のままだった)。
    if [ ! -d "$dir" ]; then
        echo "run_in_dir: ディレクトリが不正 (${dir:-空文字})" >&2
        return 1
    fi
    local saved="$PWD"
    cd "$dir" || return 1
