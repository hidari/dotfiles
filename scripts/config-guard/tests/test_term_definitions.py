...
出現のうち手当て済みのものは免除へ理由ごと記録し、新しい出現だけが報告される。
免除が実態から乖離した場合も報告する。
"""
...
def test_bare_string_defines_is_malformed() -> None:
    # ブラケット忘れ。rule_defines は空を返すので、報告しないと宣言が黙って消える
...
def test_term_declared_by_an_always_loaded_rule_is_out_of_scope(tmp_path: Path) -> None:
    # paths を持たない rules は常時ロードされるので定義が必ず届く。
    # ここを検査すると全ての出現が偽陽性になり、意味のない免除だけが増える
    root = _repo(
        tmp_path,
...
def test_malformed_defines_is_reported(tmp_path: Path) -> None:
    # 型違いは rule_defines が黙って捨てるので、報告しないと語の検査だけが沈黙する
    root = _repo(tmp_path, rule='---\npaths: ["**/*.x"]\ndefines: 架空の造語\n---\n\n# rule\n')
    findings = _check(root)
    assert [f.source for f in findings] == [f"{RULES_DIR}/testing-practices.md"]
