...
def self_identifiers(line: str) -> list[str]:
    """行が指す自リポジトリの Issue 番号。検査側は書かれた形も使うのでここで落とす。"""
...
def test_related_lines_returns_only_the_related_section() -> None:
    ...
    assert related_lines(text) == ["- Issue 1"]
...
def test_related_lines_drops_a_fenced_example_inside_the_section() -> None:
    ...
    assert related_lines(text) == ["- Issue 1", "- Issue 2"]
...
def test_issue_numbers_reads_both_notations_and_both_locations(tmp_path: Path) -> None:
    ...
    # Issue ディレクトリの下にもう 1 段ある形。剥がしすぎると番号が読めなくなる
    write_file(repo, "docs/issues/closed/9_done/notes/memo.md", "# t\n")
    run_git(repo, "add", "-A")

    assert sorted(issue_numbers(tracked_issue_markdown(str(repo)))) == ["7", "8", "9"]
...
def test_a_scope_heading_for_an_unknown_repository_is_reported(tmp_path: Path) -> None:
    # 既知のリポジトリ名に限ると、未知の名前で括った見出しだけが報告されなくなる。
    # 配下が自リポで解決される危険はむしろそちらが高い
    repo = _repo_with_related(tmp_path, "some-other-repo 側:\n\n- ISSUE-2 を参照", others=("2_b",))

    findings = check_related_refs(str(repo), baseline={})

    assert [f.detail for f in findings] == ["some-other-repo 側:"]


def test_identifiers_under_a_scope_heading_are_still_resolved(tmp_path: Path) -> None:
    # 見出しを報告するだけで配下を免除してしまうと、実在しない識別子が素通りする
    repo = _repo_with_related(tmp_path, f"{FOREIGN} 側:\n\n- Issue 99 を参照")

    details = {f.detail for f in check_related_refs(str(repo), baseline={})}

    assert details == {f"{FOREIGN} 側:", "Issue 99"}
