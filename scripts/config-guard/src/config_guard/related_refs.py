...
- 前置は同じ行のそれより後ろに在る識別子すべてに効く。他リポ前置の後ろへ自リポの識別子を
  書くと実在検査を素通りする。識別子ごとに前置を必須にする案は採らなかった。書き忘れの
  結果が誤解決の側へ倒れるため
"""
...
# (リンクテキストがディレクトリ名と一致していない実例が上流リポジトリの実測にあり、
...
# レビューが守る約束であって検査が守っている状態ではない。埋める作業は別に切り出してある。
