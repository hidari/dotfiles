...
実マッチャの検証は live probe が持ち、ここはリストの編集だけを見る。
...
# 各 rules が宣言する paths。測定の詳細は「paths は測ってから決めた」節と
...
DELIBERATELY_EXCLUDED: dict[str, str] = {
    "**/spec/**": ("テストではなく仕様の置き場だった (測定は「paths は測ってから決めた」節)"),
    "**/specs/**": (
