...
まま残り続けうるので、それを報告の条件にすると Issue が永久に閉じない。常時開いた通知は
やがて見られなくなるため、メジャー越えは単独では黙り、Issue が立つときだけ付録として載せる。
...
uv run --project scripts/mise-update-notifier mise-update-notifier \
  --config home/.config/mise/config.toml --body-out .cache/body.md
```
