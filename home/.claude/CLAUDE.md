...
- 関数型プログラミングのアプローチを強制し、厳格なエラーハンドリングパターンを守って開発すること
  - 副作用を最小限に抑え、純粋関数を優先する
  - イミュータブルなデータ操作を心がける
  - 早期リターンで深いネストを避ける
  - TypeScriptコードではany,try-catchを禁止し、常にResult/Option型を使用
  - Rustコードでは `panic!` は使用禁止、常にthiserrorによる適切なエラー型を使用
  - フロントエンド/バックエンド間でエラーハンドリングパターンを統一し一貫性をもたせる
...
### セッション開始・再開プロトコル
1. タスクに入る前にテストやLint/Formatを実施して事前状態に問題ないことを確かめる
2. これから行うタスクにはブランチが必要か判断し、必要な場合は `git-branch-switcher` を使う

### セッション終了プロトコル
1. PRマージ・タスク完了前にテストやLint/Formatを実施してコードに些細な警告もないことを確かめる
2. CIで全てのエラー・警告がない状態になったことを確認してPRマージ・タスク完了処理を行う
