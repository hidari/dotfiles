...
- スクラッチ置き場に `tmp/` ではなく `.cache/` を使うのは、Tirith の `mass_file_deletion` が `.cache` をビルド成果物として除外集合に含むため（理由: 同ルールは 20 秒窓で 3 件以上の削除を CRITICAL でブロックし、後片付けの `rm` が正当な作業中に刺さる。除外集合は tirith のソースにハードコードされていてポリシーから拡張できず、`correlate()` は policy を受け取らないので設定で回避する経路が無い。`severity_overrides` で緩めることは可能だがリポジトリスコープでは無効化され、User スコープに置くと全プロジェクトで破壊検知の強制力を失う。scratch は「まとめて消えて当然のもの」なので、除外対象の名前を使うのは抜け道ではなく意味的にも正しい。列挙はソースが canonical なので他の名前が必要になったら `tirith explain --rule mass_file_deletion` と upstream を確認する）
