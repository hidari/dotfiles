...
- Bash ツールでコマンドを組むとき、シェルの展開規則と exit code 規約を確かめてから破壊的操作や状態確認に使うこと。踏んだ 3 例: (1) このシェルは zsh なので `for x in $VAR` は単語分割されず 4 要素のリストが 1 つの名前として扱われた (2) `git check-ignore` は「ignore されていない」を exit 1 で返すため `&&` チェーンが途切れ後続の `rm -rf` が飛んだ (3) grep のパターンに `$@` を入れたら展開されて 0 件を返しファイルが壊れたと誤診した（理由: これらは例外ではなく「0 件」「何も起きない」という成功に見える形で返るので結果を見ても気づけない。破壊的操作の前は展開結果を echo で確かめ、真偽値を返すコマンドは `&&` で繋がず `;` で分ける。上の exit code ルールの裏面で、あちらは成否が隠れる側、こちらは後続が飛ぶ側）
