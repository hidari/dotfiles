...
- テストは「緑であること」だけで信用しない。各テストが (1) 仕様を表現しているか、(2) 対象を正確にテストしているかまで確かめること
  ...
  - 検査機構（ゲート・ガード・lint ルール・CI チェック）を足したときの変異は 3 種いる: (1) 検査対象を壊す (2) 検査機構そのものを壊す (3) 検査機構の「取り付け」を外す。(1) だけで完了としないこと（理由: テスト実行件数のガードを作り (1) で赤を確認して完了としたが、赤を作る唯一の行 `process.exitCode = 1` を消しても全テストが緑のまま、さらに recipe からレポータの登録を外しても全緑だった。統合テストが自前で argv を組んでいて取り付けの literal が二重管理になっていたのも同じ穴。(2)(3) を見ないと、穴が塞がったのではなく 1 段上に移動しただけになる。上の「緑 = dead pin」を検査機構自身へ適用した形）
