...
# ここで先に宣言して SC2154 (referenced but not assigned) の誤検出を防ぐ (load_usage 側は下の L130 付近で宣言済み)。
...
if [ -n "$git_branch" ]; then
  line1+="${SEP}${PURPLE}${project}${RESET} ${CYAN}${git_staged}${git_unstaged}[${git_branch}]${RESET}"
elif [ -n "$project" ]; then
  line1+="${SEP}${PURPLE}${project}${RESET}"
fi
