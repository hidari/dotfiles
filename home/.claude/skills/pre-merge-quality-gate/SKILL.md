...
4. UI 変更なら **chrome-devtools-mcp で実ブラウザ動作確認** (CLAUDE.md MUST: フロントエンド変更は実ブラウザで確認)
   - 主要ツール: `mcp__chrome-devtools__navigate_page`, `take_snapshot`, `take_screenshot`, `list_console_messages`, `click`, `hover`, `fill`
...
`gh pr merge` 成功後、 マージ PR 本文に `Closes #NNN` / `Fixes #NNN` があれば `in-repo-issue` skill の Phase C (自動クローズ) を起動する。 詳細手順は in-repo-issue SKILL.md 参照。 `gh pr create` の場合はトリガーしない。
