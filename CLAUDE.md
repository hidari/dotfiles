...
- ファイルを変更する作業の開始前に `dev-workflow:git-branch-switcher` スキルでブランチを選択する
...
- シェルスクリプトに他言語（AppleScript 等）を埋め込むときは、埋め込み側の構文検査をテストで pin すること
  - 埋め込み言語の構文エラーはホスト言語の検査を素通りし、実行するまで露見しない
  - ヒアドキュメントを変数へ受けるならコマンド置換ではなく `IFS='' read -r -d '' VAR <<'EOF' || true` を使う
    - `$(cat <<'EOF')` で受けると本文中のアポストロフィ [`AppleScript's`] をコマンド置換のパーサがクォート開始と解釈して終端を見失う
    - 変数へ受ければテストが source するだけで本文を検査でき、マーカーで切り出す text-parse も要らなくなる
  - AppleScript では `path` / `round` を変数名に使わない。予約語と衝突して -1700 / -2741 で落ちる
